    ...
    : PostgresDatabaseTestBase(postgres)
{
    ...
    public async Task UnknownChartTypeIsRejected()
    {
        ...
        var response = await client.PutAsJsonAsync(
            ...
            new
            {
                dashboardId,
                version = 0,
                layout = Layout(dashboardId, 0, "NOT_A_CHART"),
            }
        );
    ...
    public async Task CandlestickWidgetsRequireExactlyFourOhlcBindings(int bindingCount)
    {
        ...
        await using var factory = CreateFactory();
        using var client = AuthenticatedClient(factory, Guid.NewGuid());
        var layout = JsonSerializer
            .SerializeToNode(Layout(dashboardId, 0, "CANDLESTICK"))!
            .AsObject();
        layout["widgets"]![0]!["visualEncodings"]!["yAxis"] = JsonSerializer.SerializeToNode(
    ...
    public async Task CandlestickWidgetWithFourOhlcBindingsIsAccepted()
    {
        ...
        var layout = JsonSerializer
            ...
            .AsObject();
        layout["widgets"]![0]!["visualEncodings"]!["yAxis"] = JsonSerializer.SerializeToNode(
            Enumerable.Repeat("volume_mwh", 4).ToArray()
        );
    ...
    public async Task SavesAndReadsADashboardForItsActor()
    {
        ...
        var save = await client.PutAsJsonAsync(
            ...
            new
            {
                dashboardId,
                version = 0,
                layout = Layout(dashboardId, 0, title: "Operations"),
            }
        );
        ...
        Assert.Equal(
            "Operations",
            body.RootElement.GetProperty("layout").GetProperty("title").GetString()
        );
    ...
    public async Task StaleVersionReturnsCurrentServerDashboardState()
    {
            ...
            (
                await client.PutAsJsonAsync(
                    ...
                    new
                    {
                        dashboardId,
                        version = 0,
                        layout = Layout(dashboardId, 0, title: "Initial"),
                    }
                )
            ).StatusCode
        );
            ...
            (
                await client.PutAsJsonAsync(
                    ...
                    new
                    {
                        dashboardId,
                        version = 1,
                        layout = Layout(dashboardId, 1, title: "Current"),
                    }
                )
            ).StatusCode
        );
        var stale = await client.PutAsJsonAsync(
            ...
            new
            {
                dashboardId,
                version = 1,
                layout = Layout(dashboardId, 1, title: "Stale"),
            }
        );
