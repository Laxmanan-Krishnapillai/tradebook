...
public sealed class VogenDapperRoundTripTests(PostgresTestFixture postgres)
    : PostgresDatabaseTestBase(postgres)
{
    ...
    public async Task VogenValuesRoundTripThroughPostgresql17WithoutManualUnwrapping()
    {
        ...
        var expected = new DomainPrimitiveRow(
            ContractId.New(),
            Price.From(12.3456m),
            Quantity.From(-12.34567890m),
            Amount.From(-98.7654m)
        );
    ...
    private sealed record DomainPrimitiveRow(
        ContractId Id,
        Price Price,
        Quantity Quantity,
        Amount Amount
