...
public sealed class Task01IntegrityIntegrationTests(PostgresTestFixture postgres) : PostgresDatabaseTestBase(postgres)
{
    ...
    [Fact]
    public async Task Delivery_instance_is_canonical_and_invalid_month_or_instance_is_rejected()
    {
        ...
        var mismatch = await Assert.ThrowsAsync<PostgresException>(() => connection.ExecuteAsync(
            """
        ...
        var invalidMonth = await Assert.ThrowsAsync<PostgresException>(() => connection.ExecuteAsync(
            """
    ...
    [Fact]
    public async Task Audit_system_ranges_are_contiguous_and_commit_chain_is_linked()
    {
        ...
        await using (var transaction = await connection.BeginTransactionAsync())
        {
            ...
            await connection.ExecuteAsync(
                ...
                """, new { DeliveryId = deliveryId, ContractId = contractId }, transaction);
            await connection.ExecuteAsync(
                "UPDATE physical_deliveries SET volume_mwh = 12, version = version + 1 WHERE id = @DeliveryId",
                new { DeliveryId = deliveryId }, transaction);
            await transaction.CommitAsync();
        }

        var audit = (await connection.QueryAsync<AuditPeriod>(
            """
            ...
            """, new { EntityId = deliveryId.ToString() })).ToArray();
    ...
    [Fact]
    public async Task Audit_exclusion_constraint_rejects_overlapping_ranges_with_23P01()
    {
        await using var connection = new NpgsqlConnection(Postgres.ConnectionString);
        await connection.OpenAsync();
        const string insert = """
        ...
        await connection.ExecuteAsync(insert, new
        {
            ...
            common.ActorId,
            common.Operation,
            SystemStart = Utc(2026, 1, 1),
            SystemEnd = Utc(2026, 3, 1),
            ValidStart = Utc(2026, 1, 1),
            ValidEnd = Utc(2027, 1, 1),
            common.DiffPatch,
            CommitHash = new string('a', 64)
        });

        var exception = await Assert.ThrowsAsync<PostgresException>(() => connection.ExecuteAsync(insert, new
        {
            common.EntityName,
            common.EntityId,
            common.ActorId,
            common.Operation,
            SystemStart = Utc(2026, 2, 1),
            SystemEnd = Utc(2026, 4, 1),
            ValidStart = Utc(2026, 6, 1),
            ValidEnd = Utc(2027, 6, 1),
            common.DiffPatch,
            CommitHash = new string('b', 64)
        }));
        ...
        Assert.Equal(1, await connection.ExecuteScalarAsync<int>(
            ...
            new { common.EntityName, common.EntityId }));
    }

    [Fact]
    public async Task Get_entity_state_as_of_reconstructs_the_exact_historical_state()
    {
        var (contractId, _) = await CreateContractAsync();
        var deliveryId = Guid.NewGuid();
        await using var connection = new NpgsqlConnection(Postgres.ConnectionString);
        ...
        var exactMatch = await connection.ExecuteScalarAsync<bool>(
            """
        ...
        var reconstructed = await connection.ExecuteScalarAsync<string>(
            """
            SELECT get_entity_state_as_of(
        ...
        Assert.Equal((99m, "Issue", 2L), await connection.QuerySingleAsync<(decimal Volume, string Status, long Version)>(
            """
    ...
    [Fact]
    public async Task Market_prices_monthly_returns_exact_averages_for_90_daily_rows()
    {
        var start = new DateOnly(2026, 1, 1);
        var seeded = Enumerable.Range(0, 90)
            .Select(offset =>
            {
        ...
        var actual = (await connection.QueryAsync<MonthlyMarketAverage>(
            """
        ...
        Assert.Equal(90, await connection.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM market_prices"));
        Assert.Equal(
    ...
    [Fact]
    public async Task Monthly_reporting_view_joins_contract_and_counterparty_dimensions()
    {
        var (contractId, contractName) = await CreateContractAsync();
        await using var connection = new NpgsqlConnection(Postgres.ConnectionString);
        await connection.OpenAsync();
        ...
        Assert.StartsWith("Counterparty-", row.CounterpartyName);
        Assert.Equal(25m, row.VolumeMwh);
        Assert.Equal(1312.50m, row.InvoiceAmountEur);
    }

    [Fact]
    public async Task Contract_subsidy_suffix_cannot_disagree_with_status()
    {
        await using var connection = new NpgsqlConnection(Postgres.ConnectionString);
        await connection.OpenAsync();
    ...
    private async Task<(Guid Id, string Name)> CreateContractAsync()
    {
        ...
        await connection.ExecuteAsync(
            ...
            """, new { Id = contractId, Name = contractName, CounterpartyId = counterpartyId });
    ...
    private static async Task<Guid> CreateCounterpartyAsync(NpgsqlConnection connection)
    {
        ...
        await connection.ExecuteAsync(
            ...
            new { Id = id, Name = $"Counterparty-{id:N}", Shorthand = $"CP{id:N}"[..20] });
        return id;
    }
