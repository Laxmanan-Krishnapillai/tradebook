...
public sealed class DomainSurfaceArchRuleTests
{
    ...
    {
        var violations = typeof(CreateContractRequest).Assembly.GetExportedTypes()
            .Where(type => type.Namespace?.StartsWith("Tradebook.Core.Domain", StringComparison.Ordinal) == true ||
                           type.Namespace == "Tradebook.Core.DTOs")
            .SelectMany(type => type.GetProperties(BindingFlags.Instance | BindingFlags.Public))
            .Where(property => IsGuardedName(property.Name) && IsRawPrimitive(property.PropertyType))
            .Select(property => $"{property.DeclaringType!.FullName}.{property.Name}: {property.PropertyType.Name}")
            .Order(StringComparer.Ordinal)
            .ToArray();

        Assert.True(violations.Length == 0, "Raw domain primitives escaped the public surface:\n" + string.Join("\n", violations));
    }

    private static bool IsGuardedName(string name) =>
        name.EndsWith("Id", StringComparison.Ordinal) ||
        new[] { "Price", "Quantity", "Amount", "Total", "Notional", "Rate", "Volume", "Capacity", "Cost", "Revenue", "Vat", "Tariff", "Fee", "Eur" }
            .Any(name.Contains);
