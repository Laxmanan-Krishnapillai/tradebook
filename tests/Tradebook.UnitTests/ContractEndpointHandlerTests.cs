...
public sealed class ContractEndpointHandlerTests
{
    ...
    private static UpdateContractRequest UpdateRequest(Guid id) =>
        new(
            id,
            "ARLA45.SC.2601.ETSS",
            Guid.NewGuid(),
    ...
    [Fact]
    public async Task DeactivateSuccessReturns204AndForwardsAllMutationArguments()
    {
        var id = Guid.NewGuid();
    ...
    [Fact]
    public async Task DeactivateReturns404ForNotFoundWithoutFetchingCurrentState()
    {
        var id = Guid.NewGuid();
        var repository = new FakeContractEndpointRepository
        {
            DeactivateResult = MutationOutcome.NotFound,
        };
        var request = new DeactivateContractRequest(id, "Superseded by renewed contract", 3);
        var endpoint = Factory.Create<DeactivateContractEndpoint>(
            context => context.User = DomainEndpointTestData.Principal(ActorId),
        ...
        var call = Assert.Single(repository.DeactivateCalls);
        Assert.Equal(
            (id, 3L, "Superseded by renewed contract", ActorId),
    ...
    [Fact]
    public async Task DeactivateReturns409WithCurrentStateForVersionConflict()
    {
        var id = Guid.NewGuid();
