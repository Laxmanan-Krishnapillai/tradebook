...
public sealed class SemanticModelLoaderTests
{
    ...
    [Fact]
    public void ValidModelsAreLoadedAndTargetChainIsEmpty()
    {
    ...
    public void RawSqlAndInvalidIdentifiersFailLoading(string original, string replacement) =>
        AssertInvalidReplacement(original, replacement, "identifier or raw SQL");
    ...
    public void MetricExpressionsAcceptOnlyTheClosedGrammar(string original, string replacement) =>
        AssertInvalidReplacement(original, replacement, "metric expression");
    ...
    {
        using var files = ModelFiles.Create(
            ValidModel.Replace(original, replacement, StringComparison.Ordinal)
        );
        ...
        Assert.False(
            ...
            $"{invalidKind} should produce a diagnostic message"
        );
    }

    [Fact]
    public void DuplicateMemberNamesFailLoading()
    {
    ...
    [Fact]
    public void NonDateDimensionsCannotDeclareGranularities()
    {
        using var files = ModelFiles.Create(
            ValidModel.Replace(
                "{ name: root_id, entity: root, type: string, sql: id }",
    ...
    [Fact]
    public void EveryEntityMustBeReachableFromTheTarget()
    {
    ...
    [Fact]
    public void DuplicateModelNamesAcrossFilesFailLoading()
    {
    ...
    [Fact]
    public void MemberNamesCannotBeAmbiguousAcrossSemanticKinds()
    {
        using var files = ModelFiles.Create(
            ValidModel.Replace("name: value_ratio", "name: value_total", StringComparison.Ordinal)
        );
    ...
    [Fact]
    public void ReverseTraversalInvertsDirectionalOuterJoins()
    {
        ...
        Assert.Contains("FROM right_table", query.SqlText, StringComparison.Ordinal);
        Assert.Contains("RIGHT JOIN left_table", query.SqlText, StringComparison.Ordinal);
    }
    ...
    private sealed class ModelFiles : IDisposable
    {
        ...
        public static ModelFiles Create(params string[] models)
        {
            var directory = Path.Combine(
                Path.GetTempPath(),
                $"tradebook-semantic-{Guid.NewGuid():N}"
            );
