...
**Source**: Reverse-engineered, verified against operational Excel workbooks in `Tradebook_LAX/` (`Tradebook_Bioticket.xlsx`, `Tradebook_Certificates_v2.xlsx`, `Tradebook_Masterdata.xlsx`, `Tradebook_Physical.xlsm`, `Tradebook_Reports_V2_SF_integration.xlsx`)
...
> **Single source of truth for Tradebook's domain model.** Every DDL in
> `architecture/master-architecture-blueprint.md` and every implementation task in `tasks/`
> MUST derive its tables, columns, enums, and relationships from this spec.

---

## 1. Business Domain Context

Tradebook: **renewable energy certificate + physical gas trading platform** for **BioGem AS / BioGem AG** — sources, trades, sells:

1. **Guarantees of Origin (GoO)** — renewable energy certificates (REGO/GoO) from producers across Scandinavia and Europe
2. **Physical Bio-Gas (Biomethane)** — physical gas contracted via producers, sold to end customers
...
Integrates with **Salesforce (SF)**, **DENA** (German registry), **AIB** (European registry), national gas networks (GTF/Denmark, ETF/Germany, JEZ/Sweden).
...
BioGem's own legal entities acting as sourcing/sales centers. Sourced from `Company Shorthand` / `Sourcing Center` / `Sales Center` columns in Contract Overview sheets.
...
Central entity. One contract = one bilateral agreement with a counterparty for a commodity. Sourced from unified **Contract Overview** definition used identically across `Masterdata` → `ContractOverview`, `Certificates_v2` → `Contract Overview_2`, and `Bioticket` → `Contract Overview_v2`.
...
Extension of `contracts` for certificate-specific metadata (columns only in `Certificates_v2` → `Contract Overview`).
...
One record per contract per supply month — core operational line of gas and hybrid (GoO+Gas) books. Mirrors `Sourcing`/`Sales` sheets of `Tradebook_Physical.xlsm` and `Tradebook_Certificates_v2.xlsx`. `book_type` discriminates Sourcing (Buy) vs Sales (Sell) vs Intercompany legs.
...
Tax, TSO, DSO tariff rates per contract per period, in **local currency** (`Tradebook_Physical.xlsm` → `Tax`).
...
`GTF/THE - Yearly`, `GTF/THE - Monthly`, `THE/GTF - Yearly`, `THE/GTF - Monthly` price tables in `Prices` sheet are period-scoped (not daily).
...
Financial outcomes per delivery/month, supporting invoicing and SF-integrating reports (`Tradebook_Reports_V2_SF_integration.xlsx`).
...
**Delivery instance key**: every delivery-bearing entity (`physical_deliveries`, `capacity_bookings`, `transfers`, `bioticket_deliveries`) carries `contract_instance_id` = `{contract_name}-{delivery_month_no}-{year}` — natural business key used by workbooks and, downstream, Salesforce.
...
> `S`/`U` suffix on quality code MUST stay consistent with `subsidy_status` column (`SUB`/`UNS`) — suffix derived from status, never the reverse.
...
1. **Contract Uniqueness**: `contract_name` globally unique.
2. **Contract Instance**: `contract_instance_id` unique per contract per delivery month; derived as `{contract_name}-{month}-{year}`.
...
5. **Salesforce Sync**: `sf_transaction_id`, `sf_contract_name`, `sf_invoice_ref` sync via NATS JetStream outbox events. `Fact_Transactions` links SF `Certificate_Transaction__c` to Tradebook contracts via `Sales_TB_Contract` / `Purchase_TB_Contract`, detects `Sales_Month_Mismatch` / `Purchase_Month_Mismatch`.
...
10. **Ticket Unit Domain**: ticket volumes always **tonnes** (`volume_ton`) with per-ton prices (`cost_eur_ton`); gas/certificate volumes in MWh.
