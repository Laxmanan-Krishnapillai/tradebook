...
Right frame not "does human review every change" (human-in-the-loop) but "does human review after fact, at scale, only where it matters" (human-on-the-loop) — only works where compiler or automated test reliably catches drift *before* merge. Layers with fast, deterministic, hard-fail feedback safe for high AI-agent autonomy; layers with slow, external, or silent failure modes need mandatory human review regardless of how confident agent making the change is.

**Verification-loop ranking for this stack, safest to riskiest:**

1. **FastEndpoints vertical slices** — safe, high autonomy. C# compiles, DI wiring fails loudly, unit/integration tests deterministic and fast.
2. **Hangfire background jobs** — mostly safe, one carve-out. Same compiler/test net as #1, but changes to retry/idempotency logic need mandatory review — failures surface hours later in production (duplicate side effects like double charge or double email hard to unwind after fact).
3. **React/Zustand/XState UI logic** — safe-ish. TypeScript plus unit tests catch a lot; XState machines good agent territory specifically because transitions explicit and testable.
4. **TanStack Query cache reconciliation / live-query merge logic (§6.11)** — needs heavy review. Async, racy, no compiler signal. Agent can pass every test it wrote and still race in production — bug only exists in interleavings its own tests didn't happen to hit.
5. **SurrealQL PERMISSIONS/schema** — needs heaviest review, mandatory gate. No compiler involvement at all, only a live integration test catches it if one exists, and failures are silent security holes, not build breaks. **The `$auth` vs. `$token` bug found in §6.1 is the canonical example**: only a live-DB test against the real JWT access type would have caught it, and an agent (or a human) could ship that mistake with every other check green.
...
- Mandatory live-SurrealDB integration test suite gating any PR touching schema or `PERMISSIONS` files, with required human diff review on those files treated as security code, not app code.
- OpenAPI client generation wired into CI so `.NET`/frontend drift fails build outright (mirrored by schema-introspection-diff checks for direct-DB leg, per above).
- Feature flags, so agent-shipped changes go out dark with staged rollout instead of all-or-nothing.
- Staging environment with synthetic data exercising the live-query and optimistic-merge paths under load — the one area in this stack you cannot unit-test your way out of.
- Post-merge safety net: error-rate and anomaly monitoring (e.g. Sentry/OpenTelemetry across both `.NET` and SurrealDB) so races and permission mistakes slipping past review surface in minutes, not via a support ticket.
...
§6.12 identified risk ranking; this is concrete tooling to act on it, prioritized.
