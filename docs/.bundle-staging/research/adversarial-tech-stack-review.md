...
> ⚠️ Iteration-1/2 doc: DDL and diagrams use generic placeholder schema (`tenants`, `trades`, `market_ticks`, `candle_1m`) predating the Excel-verified entity model (`architecture/entity-model.md`) — single-tenant, no `trades`/`tenants` tables, `market_prices` instead of `candle_1m`. Read as illustrative complexity-reduction argument, not literal current schema.

---

## 1. Executive Summary & Context of Adversarial Review

Iteration 1 architectural exploration for Tradebook yielded highly specialized, high-throughput component recommendations for extreme scale:
- **Bi-temporal event sourcing & ledgering**: bi-temporal audit trails, Git-like branch/merge semantics, RFC 6902 JSON patch streams.
- **Polyglot CQRS databases**: SurrealDB for direct-to-browser GraphQL/SurrealQL live query reads and RLS mutations, PostgreSQL for relational domain storage, ScyllaDB for ultra-high-throughput ledger appends, ClickHouse for vectorized OLAP analytics, S3 WORM Parquet for long-term historical archives.
- **Polyglot service layer**: .NET 9 FastEndpoints alongside high-frequency microservices in Rust, Apache Kafka/Redpanda for CDC event streaming, Redis distributed caching, Hangfire background job scheduling.
- **Container orchestration**: multi-region Kubernetes (EKS/GKE) clusters, complex service meshes, custom operators.

These choices theoretically support sub-ms query latencies and linear horizontal scaling to millions of concurrent users. Unconstrained **Adversarial Tech Stack & Complexity Review** reveals a fundamental flaw: **over-engineering for hypothetical hyper-scale at the expense of real-world operational viability, developer velocity, financial sustainability, and system security**.

### 1.1 Core Philosophy & Fixed Architectural Boundaries
Primary mandate: challenge every unnecessary layer of complexity — why fragment state and logic across multiple specialized databases and languages when modern unified engines handle the workload? Evaluate against **90/10 Engineering**: *90% of theoretical maximum capabilities, 10% of operational overhead*.

**Fixed Non-Negotiable Constraint**:
- **Backend Stack**: **.NET 9 / 10 (C# / ASP.NET Core)** is a **non-negotiable organizational requirement**. All backend API endpoints, domain logic, background jobs, real-time streaming interfaces must be natively C# / .NET.

### 1.2 Summary of Key Findings & Recommendations
1. **Database Hyper-Fragmentation**: maintaining 5 stateful database systems (PostgreSQL, SurrealDB, ScyllaDB, ClickHouse, Redis) introduces severe CDC sync lag, risks split-brain data drift across stores, requires dedicated SRE maintainers.
2. **SurrealDB Production Hazards**: severe disaster recovery bottlenecks (sequential text SQL replay >7 hours for 200k records), memory expansion under live-query fan-out (`#5068`, `#7358`), critical RLS permission-bypass advisories.
3. **The Power of Consolidation**: **PostgreSQL 17** (+ **TimescaleDB** for time-series continuous aggregates and bi-temporal range indexing) + **NATS JetStream** (lightweight pub/sub + KV state) consolidates **all database and messaging needs** into a single primary system of record.
4. **Optimized .NET 9 Monolith**: **Pragmatic .NET 9 Modular Monolith** (Native AOT, FastEndpoints/Minimal APIs, `HybridCache`, `System.Threading.Channels`, SignalR Core) achieves **elite throughput (>35,000 req/sec per node)** and **sub-30MB RAM footprints**, eliminating polyglot Rust/Go microservice ops overhead.
5. **Linear/Twenty-Grade Ultra-Snappy UI**: achieved via **Optimistic Mutations** on frontend (TanStack Query + IndexedDB Dexie mutation queue) + **SignalR Core binary MessagePack push** + **.NET 9 compiled queries / HybridCache L1 memory tiering** on backend.
...
**Verdict**: given .NET 9 as non-negotiable foundation, **a .NET 9 Modular Monolith compiled with Native AOT delivers world-class execution performance, sub-50ms latency, drastically lower cognitive load** vs polyglot microservices.
...
**Verdict**: ScyllaDB introduces extreme operational overhead and query rigidity. **PostgreSQL 17 wins for relational flexibility, bi-temporal data integrity, and ACID transaction guarantees**.
...
**Verdict**: Redpanda improves on Kafka by removing the JVM, but **NATS JetStream** is optimal for Tradebook — pub/sub messaging, distributed KV caching, WebSocket client streaming in one low-memory (<50MB) Go binary pairing seamlessly with .NET 9 (`NATS.Client.Core`).
...
**Verdict**: ClickHouse forces a secondary database cluster and continuous CDC ETL sync. **TimescaleDB keeps all time-series/analytical queries inside PostgreSQL**, eliminating cross-database sync lag and dual-cluster maintenance.

---

### 2.5 SurrealDB + Multi-Service Setup vs. Consolidated PostgreSQL 17 + .NET 9 Monolith

Critique of the Iteration 1 proposal:
...
1. **Disaster Recovery Backup Bottleneck**: SurrealDB's only backup mechanism exports SQL text (`.surql`), replayed sequentially on restore. Restoring modest datasets (200k records) documented taking >7 hours — unacceptable RTO risk.
2. **Security & Row-Level Security (RLS) Churn**: multiple permission-bypass advisories published in 2026 affect row-level permissions — the exact mechanism relied on for tenant isolation when web clients connect directly to SurrealDB via WebSockets.
3. **Live Query Memory Leak & Fan-Out Bottleneck**: SurrealDB live query buffers expand rapidly under high fan-out, causing process hangs (`#5068`) and query execution starvation (`#7358`).
4. **Licensing Restrictions**: SurrealDB's Business Source License (BSL 1.1) limits DBaaS redistribution, long-term licensing risk for enterprise deployments.

---

## 3. Mathematical Complexity Reduction Scoring Model (CRS)

To objectively evaluate architectural simplification under the **non-negotiable .NET 9 requirement**: formal **Complexity Reduction Scoring Model (CRS)**, 1–100 scale, five weighted operational categories.

### 3.1 Formal Mathematical Formulation

Let $C$ be Total Complexity Score of an architectural stack:
$$C = \sum_{i=1}^{5} w_i \cdot S_i$$

Where:
- $w_i$ = weight assigned to category $i$, $\sum_{i=1}^{5} w_i = 1.00$.
- $S_i \in [1, 100]$ = raw complexity sub-score for category $i$ ($1 = \text{minimal complexity}$, $100 = \text{extreme complexity}$).

**Complexity Reduction Score ($CRS$)** comparing Baseline Stack ($C_{\text{base}}$) vs Pragmatic .NET 9 Stack ($C_{\text{alt}}$):
...
> **Quantitative Conclusion**: Pragmatic .NET 9 Tech Stack achieves **70.29% reduction in overall system complexity** while strictly fulfilling non-negotiable .NET organizational mandate and satisfying 100% of Tradebook's functional/operational goals.
...
Major question raised by replacing SurrealDB direct-to-browser WebSockets: **how does this simplified .NET 9 + PostgreSQL architecture guarantee a Linear / Twenty CRM style sub-50ms snappy CRUD experience?**

End-to-end latency budget and technical execution plan below, proving how the simplified .NET 9 architecture achieves instant responsiveness.
...
- **0ms Perceived Latency**: user updates a record/cell/reorder → frontend immediately mutates local TanStack Query cache, renders update within **1 frame (16ms @ 60Hz)**.
- **IndexedDB Mutation Queue**: pending edits written to offline-resilient IndexedDB mutation queue via Dexie.js. Network drop → mutations queue locally, auto re-sync on reconnection.
- **Optimistic Concurrency Control**: .NET 9 endpoints check PostgreSQL system column `xmin`/version token. Conflict → .NET returns HTTP 409 Conflict with server delta, client resolves cleanly without full page reloads.

#### Pillar 2: High-Performance .NET 9 Backend Execution Engine
- **Native AOT Compilation**: compiling .NET 9 Web API with Native AOT (`<PublishAot>true</PublishAot>`) removes JIT compilation overhead, reduces startup cold-starts to **<5ms**, caps memory footprint at **<30MB RAM**.
- **Zero-Allocation Hot Paths**: endpoints use `ReadOnlySpan<char>`, `ValueTask<T>`, `System.IO.Pipelines`, memory pooling (`ArrayPool<T>`) to eliminate GC pause spikes during heavy editing sessions.
- **Compiled EF Core / Dapper Queries**: hot-path SELECT/UPDATE bypass LINQ compilation overhead via `EF.CompileAsyncQuery` or raw Dapper SQL over `NpgsqlDataSource` with multiplexed connection pooling.

#### Pillar 3: Multi-Tier Caching with .NET 9 `HybridCache`
- **L1 In-Memory & L2 Pub/Sub Caching**: .NET 9's `HybridCache` combines fast in-memory L1 cache (sub-microsecond response) with distributed L2 caching (NATS JetStream / Redis).
- **Instantaneous Read Views**: complex aggregated views/dashboard metrics served directly from .NET L1 HybridCache in **<0.5ms**, invalidating granular cache keys only when PostgreSQL outbox events fire.

#### Pillar 4: SignalR Core Binary Push & Backpressure Management
- **High-Frequency WebSocket Sync**: instead of heavy polling or fragile direct DB WebSockets, clients connect to ASP.NET Core **SignalR Hubs** using binary **MessagePack** protocol (`Microsoft.AspNetCore.SignalR.Protocols.MessagePack`). MessagePack payloads up to **70% smaller** than raw JSON — drastically reduces bandwidth and client parsing CPU.
- **Backpressure via `System.Threading.Channels<T>`**: high-volume change broadcasts (market tick streams, audit logs) flow through bounded `.NET Channels` (`Channel.CreateBounded<T>`), preventing memory spikes broadcasting to thousands of connected clients.
