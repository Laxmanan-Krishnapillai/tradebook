...
Tradebook needs a flexible, high-performance visual analytics engine serving two personas:
1. **Executive & Operational Users**: expect clean, responsive, aesthetic summary dashboards, instant load times, clear KPI cards, intuitive filter controls.
2. **Quantitative & Financial Traders**: demand real-time, high-density financial charting (candlesticks, order book depth, tick streams, volume profiles, heatmaps) rendering thousands of data points at 60 FPS without main-thread freeze.

To avoid code bloat and vendor lock-in, Tradebook decouples dynamic visual presentation from data fetching via a **plug-and-play visual framework**: semantic model binding layer, dynamic JSON layout engine, offscreen canvas web worker rendering pipelines, extensible widget plugin architecture.
...
* **R4.1 Dynamic Self-Service Visual Building**: users create/configure/modify dashboard widgets via drag-and-drop without deploying code or altering backend endpoints.
* **R4.2 Semantic Model Integration**: visual encodings (X/Y axes, color scales, legend groupings, tooltips, drill-downs) bind directly to Pillar 2 semantic metrics/dimensions.
* **R4.3 Real-Time Streaming Performance**: financial tick streams/live order updates render smoothly at 60 FPS. High-density tick datasets (up to 1,000,000 points) use off-main-thread processing (Web Workers + `OffscreenCanvas`).
* **R4.4 Cross-Widget Interactivity**: inter-widget communication (cross-filtering, linked zooming, temporal sync, hover highlights) via decoupled Event Bus with microsecond latency.
* **R4.5 Extension & Isolation**: third-party/domain-specific custom widgets pluggable via dynamic registration API with explicit sandboxing boundaries.
...
Selecting charting tech requires evaluating rendering mechanisms (SVG vs Canvas vs WebGL vs HTML), financial domain readiness, dynamic responsiveness, bundle impact, DX.
...
* **Architecture**: React-native component UI library on top of Tailwind CSS and Recharts primitives.
* **Strengths**: Out-of-box visual elegance matching modern SaaS tools (Linear, Vercel). Seamless Radix UI/Tailwind v4 integration. Ideal for KPI cards, delta badges, executive metrics.
* **Weaknesses**: SVG rendering limits performance to <5,000 points per chart. No native financial chart types.
* **Tradebook Role**: **Tier 1 Executive Summary Engine** (KPI cards, mini-sparklines, portfolio summary callouts).

#### B. Nivo
* **Architecture**: Comprehensive React charting collection, SVG/Canvas/Server-Side HTTP rendering modes on D3 interpolation primitives.
* **Strengths**: Rich declarative React API with Framer Motion transitions. Excellent treemap, bump chart, chord diagram capabilities.
* **Weaknesses**: SVG components suffer high memory under continuous real-time streams. Canvas implementations need manual tooltip event handling.
* **Tradebook Role**: **Specialized Diagram Engine** (hierarchical portfolio treemaps, flow breakdown sankey diagrams).

#### C. Apache ECharts
* **Architecture**: Industrial-grade Apache Software Foundation engine, ZRender 2D execution framework supporting Canvas/WebGL/SVG.
* **Strengths**: Handles up to 1,000,000 points smoothly via WebGL/Canvas. Comprehensive declarative JSON option model. Built-in data zoom, brush select, visual mapping, multi-axis coordination.
* **Weaknesses**: Imperative `setOption` updates need careful React wrapper lifecycle management to prevent leaks. Large bundle if untree-shaken.
* **Tradebook Role**: **Tier 2 Core Analytical Engine** (multi-axis performance charts, risk heatmaps, trade execution scatter plots, dashboard builder default).

#### D. TradingView Lightweight Charts
* **Architecture**: Zero-dependency 2D Canvas engine optimized exclusively for high-frequency financial time-series.
* **Strengths**: Unmatched candlestick/volume/line/tick performance. Smooth hardware-accelerated pan/zoom/crosshair. Ultra-small footprint (~45 KB).
* **Weaknesses**: Domain-specific to financial time-series. No non-time-series charts (pie, treemap, scatter).
* **Tradebook Role**: **Tier 3 High-Frequency Financial Engine** (live trading view, order execution charts, market depth).

#### E. Observable Plot
* **Architecture**: Concise D3-creator-authored library, declarative "grammar of graphics" (marks, scales, transforms).
* **Strengths**: Exceptional rapid exploratory data analysis, statistical distributions, ad-hoc visual transforms.
* **Weaknesses**: SVG-bound. No interactive React state management or financial streaming primitives out of the box.
* **Tradebook Role**: Internal exploratory analysis tool; not selected for core dashboard runtime.

---

### 2.3 Rendering Pipeline Performance & Threading Architecture

Rendering high-frequency tick data or >100,000-point historical data on a single JS thread causes UI lag/micro-stutters. Tradebook adopts an **Off-Main-Thread Data Transformation and Offscreen Canvas Pipeline**.
...
To preserve visual fidelity without rendering millions of invisible overlapping elements, the Web Worker applies **Largest-Triangle-Three-Buckets (LTTB)** to downsample high-density time-series to the chart's exact pixel width (e.g. 1,200 points for a 1,200px display).
...
For charts supporting `OffscreenCanvas` (Apache ECharts, custom Canvas series), DOM control transfers off the main thread:
...
Browsers (Chromium, Firefox, WebKit) enforce a strict hard limit on active WebGL contexts per domain (typically 16 max). Exceeding it triggers silent eviction of older contexts or context loss errors (`WEBGL_CONTEXT_LOST_WEBGL`), resulting in black screens or crashes.

To prevent VRAM leaks and context-lost errors, Tradebook establishes three explicit GPU memory governance mechanisms:

1. **WebGL Canvas Context Pooling**: for mini-sparklines/small analytic charts, reuses a pooled set of shared 2D/WebGL contexts instead of dedicated contexts per widget instance.
2. **Hard Cap of Max 8 Active Canvas Widgets per Tab**: when a dashboard layout has >8 canvas widgets:
   - Widgets outside the active viewport render static SVG/PNG preview fallbacks via `IntersectionObserver`.
   - Scrolling/tab switching triggers dynamic context allocation from the pool for visible widgets, releasing off-screen contexts.
3. **Mandatory React Component Unmount Disposal Hooks**: all visual React wrappers (ECharts, TradingView, custom Canvas) must execute explicit `.dispose()`, `.clear()`, or `chart.remove()` in `useEffect` cleanup to release VRAM and return contexts to the pool.
...
Running multiple client-side data/analytics engines simultaneously (TanStack DB from Pillar 3, DuckDB WASM from Pillar 2, Web Worker visualization buffers from Pillar 4) risks browser memory bloat exceeding 1.5–2.5 GB RAM, triggering tab crashes (OOM) or background eviction.

Tradebook reconciles all browser data stores under a **Single Unified Client Memory Budget** capped at **512 MB per tab** (ultra-constrained 256 MB profile for mobile/low-memory devices).
...
Dashboards stored as structured JSON configs, decoupling layout grid dimensions, visual theme overrides, and widget semantics from frontend React code.
...
When a user selects a date range on a candlestick chart, filters an asset class on a bar chart, or hovers a trade scatter plot, all relevant dashboard widgets must update synchronously or filter their display.
...
To enable custom visualizations from internal teams or third-party vendors (custom risk model charts, algorithmic execution diagrams), Tradebook provides a **Dynamic Visual Plugin Architecture**.
...
2. **Internal Enterprise Custom Widgets**: encapsulated using **Web Components + Shadow DOM** for style encapsulation with zero memory overhead.
3. **Untrusted External 3rd-Party Extensions**: sandboxed within `<iframe>` wrappers using an async `postMessage` protocol for data/event bus transfer.
...
**Metabase and Lightdash are rejected** as the primary visualization solution — neither supports real-time WebSocket tick streams, order book depth charts, or custom financial canvas rendering.

Tradebook builds an **In-House Dynamic Dashboard Engine**: Tremor for KPI summary cards, Apache ECharts for complex multi-axis analytics, TradingView Lightweight Charts for financial market data.
...
Tradebook adopts a **3-Tier Visual Component Strategy** for optimal performance, aesthetic consistency, maintainability:
...
Final architecture connects real-time data ingestion, Pillar 2 semantic query engine, offscreen worker data processing, dynamic visual encoding mappers, and the 3-tier display layer.
