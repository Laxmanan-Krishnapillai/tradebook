...
> ⚠️ Early-iteration doc: schemas use `tenant_id` throughout and generic domain entities (`workflow_nodes`, `kanban_card`) — the Excel-verified entity model (`architecture/entity-model.md`) is explicitly **single-tenant** (no `tenants` table) with BioGem-specific entities (`contracts`, `physical_deliveries`, etc). SurrealDB read-model role is consistent with `master-architecture-blueprint.md` §2.1 (still present as read-only projection). Read as illustrative versioning/audit pattern, not literal current schema.

---

## Executive Summary & System Context

Tradebook: high-performance, real-time B2B app combining interactive workflow automation canvases (`@xyflow/react`), kanban task management (`@dnd-kit`), visual data management, dynamic analytical dashboards. Enterprise financial/operational requirements demand **full revertability**, **granular change attribution ("who changed what, when, why, and from where")**, **tamper-proof compliance recording**, **collaborative workspace branching/merging**.

### Baseline Architectural Stack & Topology

Strict CQRS-split hybrid application architecture, PostgreSQL as single primary write authority:
...
- **Frontend Layer**: React 19 SPA via Vite, `@tanstack/react-router`, `Zustand` (UI state), `XState` (workflow machines), `@xyflow/react` (canvas), `@dnd-kit` (drag-and-drop), `@tanstack/react-virtual` (virtualized tables).
- **Backend Layer**: .NET 9 Web API, **Vertical Slice Architecture**, `FastEndpoints` (REPR pattern) + `FluentValidation`.
- **Primary Write Authority**: `PostgreSQL` — single primary write store for all domain entities, bi-temporal `audit_log` tables, transactional outbox tables, background `Hangfire` jobs.
- **Read Model & Streaming Engine**: `SurrealDB` strictly a read-model + real-time push engine, synced asynchronously via CDC outbox workers. Direct writes to SurrealDB from API endpoints or frontend clients prohibited.
- **Cold Storage**: AWS S3 Parquet Lakehouse, S3 Object Lock (`COMPLIANCE` retention mode), tamper-proof non-repudiable audit archives.
- **Single Write Authority CQRS Enforcement**: direct browser database writes strictly restricted (`PERMISSIONS FOR create, update, delete NONE` on SurrealDB tables). All state mutations execute as atomic PostgreSQL transactions inside .NET 9 FastEndpoints (updating entity state, writing bi-temporal audit logs with RFC 6902 JSON-Patch diffs, emitting outbox events). Single-write topology prevents dual-write split-brain data drift, ensures 100% audit coverage.
...
1. **Valid Time (`valid_time`)**: real-world business period a fact is true (e.g. trade contract valid `2026-01-01` to `2026-12-31`).
2. **System Time (`system_time`)**: transaction timeline recorded by the DB engine — when the record was physically stored/modified.

Schema below uses `TSTZRANGE` ranges, `btree_gist` extension for composite temporal exclusion constraints across both timelines (`system_time WITH &&` and `valid_time WITH &&`), JSONB RFC 6902 patch tracking.
...
SurrealDB serves as Tradebook's operational read-model and real-time push engine. Audit logs in SurrealDB stored in append-only SCHEMAFULL tables protected by strict Record-Level Security (RLS).
...
For high-performance serialization across the CDC pipeline, microservices, and S3 cold storage compaction, audit payloads defined using Protocol Buffers v3 (`audit_payload.proto`).
...
In pure Event Sourcing, aggregate state never stored directly; reconstructed on-demand by replaying an immutable stream of domain events.
...
Tradebook establishes **PostgreSQL as sole primary write authority** via Transactional Outbox + CDC pipeline. State writes, bi-temporal audit logs, outbox event insertions execute inside a single atomic PostgreSQL transaction in .NET 9 FastEndpoints. Debezium/Hangfire workers tail outbox records, syncing SurrealDB read views and publishing audit payloads to Kafka and S3. Direct database writes from clients or dual-writes to SurrealDB from API endpoints strictly prohibited.
...
To comply with non-repudiation mandates (SEC Rule 17a-4, FINRA, GDPR audit compliance), cold audit records must be physically unalterable.
...
- **S3 Object Lock Configuration**:
  - **Bucket Policy**: enforces `COMPLIANCE` retention mode, 7 years. In `COMPLIANCE` mode no user — including AWS root account — can delete or modify object versions until retention expires.
  - **Legal Hold**: togglable per tenant folder for ongoing litigation/regulatory inquiries.
- **Partition Layout**:
  ```text
...
Every audit log entry contains a cryptographic hash linking it sequentially to its predecessor. Batches of audit events compiled into **Merkle Trees** for constant-time tamper detection across millions of records.

To prevent cryptographic collision flaws (CVE-2012-2459, the Bitcoin Merkle tree leaf duplication vulnerability), Tradebook constructs Merkle trees strictly per **RFC 6962 (Certificate Transparency standard)**:
1. **Leaf Node Hashing**: prefix byte `0x00` prepended to leaf data before SHA-256: `SHA-256(0x00 || protobufEventBytes)`.
2. **Internal Node Hashing**: prefix byte `0x01` prepended to combined child hashes before SHA-256: `SHA-256(0x01 || leftChildHash || rightChildHash)`.
3. **Odd Node Carry-Up**: tree level with odd number of nodes → last node carried up directly to next level without duplication.
...
High-velocity audit logs in PostgreSQL/Kafka compacted by background `Hangfire` recurring job into compressed Apache Parquet files on S3.

1. **Extraction**: Hangfire queries unprocessed entries in `audit_log` older than 1 hour.
2. **Serialization**: convert entries to Protobuf, write to Apache Parquet columnar format using `ZSTD` compression. Parquet columnar indexing enables instant filtering on `tenant_id`, `entity_type`, `actor_id` via DuckDB or AWS Athena.
3. **Verification**: generate RFC 6962 Merkle Tree root hash for the Parquet chunk, store block header manifest.
4. **Flush & Prune**: upload chunk to S3 WORM bucket; mark Postgres rows `archived=TRUE`.
...
To support collaborative drafting (branching a workflow canvas, editing, merging back to production), Tradebook implements a recursive 3-Way Merge engine based on RFC 6902 JSON-Patch deltas.
...
1. **Recursive Deep Matching**: merges nested objects/child collections recursively, not shallow top-level key comparison.
2. **Stable ULID Key Alignment**: arrays of entities (canvas nodes, kanban cards) matched via stable ULID entity keys (`id`) instead of positional array indices, preventing false conflict storms when array order shifts.
3. **Non-Destructive `FAIL` Strategy**: under `FAIL` strategy, conflict states **do NOT overwrite target or source data**. Conflicted field paths isolated in `conflicts[]`, flagging `success: false` without destroying existing state.
...
Expanded matrix evaluates six core architectural paradigms across storage, latency, query capability, auditability, regulatory compliance, write amplification, schema evolution costs, operational complexity.
...
Tradebook implements a **PostgreSQL Primary Write Authority with Hybrid CDC Outbox + Bi-Temporal JSONB Audit Architecture** and **Git-Style Branching for Workflow Canvases**:
...
1. **Write Pipeline (.NET 9 FastEndpoints & PostgreSQL)**: enforces single write authority entry point using an ASP.NET Core `AuditInterceptor`. Captures authenticated `tenant_id`, `actor_id`, client IP, user-agent, correlation ID, pre/post entity states inside an atomic PostgreSQL transaction.
2. **Operational Datastore (PostgreSQL + SurrealDB)**:
   - PostgreSQL stores relational domain entities, bi-temporal `audit_log` records (composite `system_time WITH &&, valid_time WITH &&` exclusion constraints), transactional outbox entries.
   - SurrealDB mirrors live state asynchronously via CDC outbox workers for low-latency browser queries (`SELECT`) and real-time push subscriptions (`LIVE SELECT`). Direct writes to SurrealDB prohibited.
3. **Audit Pipeline (Hangfire + S3 WORM)**: Hangfire background workers extract outbox records, construct RFC 6962 SHA-256 Merkle trees (0x00 leaf prefix, 0x01 internal node prefix, odd node carry-up), archive compacted Parquet files to AWS S3 Object Lock storage.
...
- Implement .NET 9 `FastEndpoints` global filter `AuditInterceptor` to capture incoming request context, execute pre/post state diffing via `JsonDiffPatchDotNet`, write audit records transactionally to PostgreSQL.
...
- Build Hangfire recurring job (`AuditCompactionJob`) running hourly to aggregate outbox logs, calculate RFC 6962 Merkle tree root hashes, write compressed Parquet blocks to AWS S3 with Object Lock (`COMPLIANCE` retention mode).
...
- Build backend 3-Way Merge engine (`perform3WayMerge`) in .NET / TypeScript to support visual canvas branching, diff previewing, conflict resolution using recursive RFC 6902 JSON-Patch merging and ULID array keys.
...
This architectural specification provides Tradebook an uncompromised, production-ready foundation for versioning, temporal queries, non-repudiable auditing, collaborative branching.

### Verification Commands & Methods
1. **Schema Verification**: execute `psql -f audit_schema.sql`, verify GIST composite temporal exclusion constraints against overlapping valid/system ranges (`system_time WITH &&, valid_time WITH &&`).
2. **Merkle Proof Verification**: run .NET test suite `dotnet test --filter Category=AuditMerkleTests` to validate RFC 6962 leaf hash calculation (0x00 prefix), internal node calculation (0x01 prefix), odd-node carry-up without duplication, proof path evaluation.
3. **Audit Interceptor Integration**: execute FastEndpoints API integration tests confirming 100% of POST/PATCH/DELETE endpoints execute atomic PostgreSQL write transactions populating `audit_log` with valid JSON patches and actor contexts before CDC relay.
4. **3-Way Merge Verification**: run Jest / Vitest test suite `npm test mergeEngine.test.ts` to verify recursive JSON-Patch merging, stable ULID key matching for array elements, non-destructive `FAIL` strategy conflict isolation.
