...
| 1 | **14** Backend analyzers & compile-time safety | 13 | local/cloud | high | Analyzers-as-errors, `[OptionsValidator]`, Mapperly. |
| 1 | **15** Strongly-typed primitives (Vogen) | 13 (coord 16) | local/cloud | high | Land before 16/20/22 consume the value objects. |
| 1 | **17** Wolverine messaging & durable outbox | 13 (**supersedes 03 outbox**) | local | high | Replaces existing outbox → regression risk; lean on integration tests. |
...
| 2 | **16** Contract-first API — TypeSpec/OpenAPI/TS client | 13 (**supersedes 08 TypeGen**; rel 15,17) | local/cloud | **xhigh** | Very High. Money-as-string; SignalR payload validation. |
