...
### 1.1 The Shift from Human Ergonomics to Agent-First Ergonomics
In traditional software engineering, developer tools and workflows are optimized for human cognitive patterns—favoring visual feedback, permissive compiler flags, interactive debuggers, and informal commit messages. However, when software development is driven by **autonomous AI coding agents** (e.g., Gemini, Claude, and dedicated subagent pipelines), human ergonomics fail to provide the deterministic feedback required for error-free software generation.

AI agents excel when operating within an environment defined by **Agent-First Ergonomics**:
- **Deterministic Feedback Loops**: Fast, machine-parseable static analysis, type checking, and test output that immediately notify agents of regressions.
- **Contract Enforcement**: Strict, automated schema synchronization between backend and frontend to eliminate ambient runtime type errors.
- **Hermetic Isolation**: Isolated, side-effect-free execution environments that ensure tests pass or fail solely based on code changes rather than external state.
- **Context Clarity**: Explicit context maps (`AGENTS.md`, `GEMINI.md`) that bound agent reasoning and prevent hallucinated architectural imports.
- **Reproducible Infrastructure**: Infrastructure-as-code with compile-time validation rules and local containerized runtime definitions.

### 1.2 The 5 Pillars of Agent Readiness
The Tradebook Agent-Readiness Framework is built upon 5 operational pillars:
1. **Conventional Commits 1.0.0 & Monorepo Semantic Release Pipeline**: Machine-parsable git histories, strict scope enforcement, and automated semantic versioning.
2. **Automated Type-Safety Contract Generation**: Zero-drift C# REPR DTO to TypeScript interface compilation via TypeGen and FastEndpoints OpenAPI.
3. **Hermetic Test Fixtures & Stryker Mutation Testing Guardrails**: Disposable Testcontainers PostgreSQL 17, sub-10ms Respawn resets, MSW 2.0 frontend network mocks, and Stryker mutation score gates.
4. **Modular Component Boundaries & AI Context Map System**: ESLint boundary enforcement, root `AGENTS.md` context maps, and `GEMINI.md` system directives.
5. **Deterministic Infrastructure as Code & Local Docker Compose Environment**: Self-contained docker-compose stack with PostgreSQL 17 extensions and fail-fast Terraform HCL validation protocols.
...
Autonomous agents require structured git commit conventions to allow automated changelog generation and semantic release triggers. All commits must follow **Conventional Commits 1.0.0**:
...
- `feat`: A new end-user or API feature.
- `fix`: A bug fix.
- `docs`: Documentation updates or context map updates.
- `style`: Formatting, missing semi-colons, white-space fixes (no production logic change).
...
To enforce clean separation in monorepo commits, allowed scopes are mapped directly to physical monorepo boundaries:
...
Automated semantic versioning analyzes commits on `main`, bumps version tags, builds release notes, and updates `CHANGELOG.md`.
...
To ensure autonomous AI agents never generate invalid commit strings that fail CI git hooks, agents use this executable wrapper:
...
Maintaining type alignment between a C# backend (.NET 9 FastEndpoints) and a React 19 TypeScript frontend is critical for AI agent productivity. If an AI agent modifies a C# DTO without updating frontend types, runtime bugs emerge.
...
The framework implements a **Dual-Layer Contract Pipeline**:
1. **TypeGen Layer**: Compiles pure C# DTO models and Enums directly to `src/frontend/src/types/generated`.
2. **FastEndpoints OpenAPI + Orval Layer**: Emits OpenAPI 3.1 JSON and compiles strongly-typed React Query hooks (`useCreateTrade`, `useGetLedger`).
...
This GitHub Actions workflow ensures that any PR modifying backend DTOs without committing updated TypeScript types fails CI immediately:
...
Autonomous AI agents require tests that execute deterministically without ambient dependency failures. 

The strategy combines:
1. **Testcontainers PostgreSQL 17**: Ephemeral Docker containers instantiated per test fixture class.
2. **Respawn Table Resets**: High-performance PostgreSQL table truncation running in sub-10ms between test cases.
3. **MSW 2.0 (Mock Service Worker)**: Network-level HTTP mocking for React 19 integration tests to prevent flaky network dependencies.
...
Stryker mutates source code binary trees (e.g., swapping `>` for `<`, replacing `&&` with `||`) to verify that tests actually fail when logic is altered.
...
Different architectural modules enforce strict mutation survival thresholds. If a build PR drops below the threshold, the build is blocked.
...
To prevent autonomous agents from cross-importing private module implementations, the repository enforces strict ESLint boundaries.
...
The root `AGENTS.md` acts as the primary navigational context for all AI subagents:
...
The `GEMINI.md` file defines explicit system-level instructions for Gemini-family agents interacting with the repository:
...
Autonomous agents require a deterministic local stack to execute tests and inspect runtime behavior.
...
Terraform configurations enforce fail-fast validation rules on all input parameters so agents cannot deploy invalid infrastructure configurations.
...
When an autonomous AI agent modifies or creates Terraform infrastructure modules, it MUST execute this 3-step verification sequence prior to proposing changes:
