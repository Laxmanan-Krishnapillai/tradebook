...
Modern high-productivity B2B web applications—exemplified by **Linear**, **Twenty CRM**, **Notion**, and **Figma**—have redefined user expectations for web UI responsiveness. Users no longer tolerate loading spinners, modal blocking, or network latency during basic CRUD operations (create, read, update, delete). To achieve "snappy" interaction, an application must decouple local user interactions from network roundtrips, targeting a **sub-100ms perceived CRUD latency** and a **0ms local UI response time**.

This document presents an exhaustive technical blueprint for Tradebook’s high-performance CRUD UI/UX stack. It covers:
1. **Perceptual UX Benchmarks & Design Patterns**: Deconstructing Linear and Twenty CRM to establish Tradebook's latency budget and keyboard-first action engine.
2. **Local-First & Sync Engine Architecture**: Complete TypeScript specifications for an IndexedDB mutation queue, Command Pattern undo/redo stack, local-first engine trade-off matrix, and sequence diagrams for optimistic CQRS write reconciliation.
3. **High-Performance UI Rendering & Unified State Management**: Data grid virtualization analysis (AG Grid vs TanStack Virtual vs Canvas/HTML5), React Flow + dnd-kit zoom-aware transform translator, and a unified state architecture integrating Zustand, XState, and TanStack Query / TanStack DB.
4. **Comprehensive Comparative Trade-Off Matrix**: Matrix comparing local-first engines and virtual grid engines across 9 core architectural dimensions.
5. **Technology Recommendations & Implementation Roadmap**: Clear guidance establishing **Decision A** (TanStack DB pilot over SurrealDB live query WS stream) and **Decision B** (PostgreSQL + ElectricSQL/PowerSync fallback), accompanied by a 4-phase execution plan.
...
Human perceptual psychology establishes distinct latency thresholds for computer interfaces:
- **< 16.6ms (1 Frame @ 60 Hz)**: Perceived as instantaneous, physical touch response. Necessary for cursor movement, drag-and-drop node movement, and typing.
- **< 100ms**: Perceived as immediate reaction. Users feel they are directly manipulating data without software hindrance.
- **> 300ms**: The human brain experiences a context switch, breaking flow state.

To guarantee sub-100ms CRUD operations regardless of global network jitter or database disk I/O, Tradebook mandates an **Optimistic Local-First Architecture**. Under this model, the client UI mutates local state immediately (0ms), enqueues a background sync payload, and handles server reconciliation asynchronously.
...
* **Optimistic Local Store**: Linear maintains a complete in-memory graph of client-accessible issues using a local SQLite/IndexedDB cache. All writes update local state in <5ms.
* **Global Action Engine (`cmdk`)**: Keyboard shortcuts (e.g., `C` for create, `K` for command palette, `Cmd+Z` for undo) bypass mouse navigation entirely.
* **Transient Undo Toasts**: Destructive operations (e.g., deleting an issue or changing state) display a transient toast containing an instant "Undo" button backed by an inverse client command.

#### B. Twenty CRM (Open-Source Relational CRM)
* **Metadata-Driven Data Grids**: Views, columns, and filters are rendered dynamically based on metadata schemas without hardcoded React layout code.
* **Inline Spreadsheet-Style Editing**: Clicking a grid cell opens an overlay editor (text, select, relation picker) that saves on blur or `Enter` without requiring explicit "Save" button clicks.
* **Bulk Command Operations**: Multi-select checkboxes trigger a sticky bottom action bar for batch updating or batch archiving records.

#### C. Notion & Figma (Block & Canvas Engines)
* **Block / Node Level Optimism**: Graph nodes (React Flow) and blocks update local position and parent-child relations instantly.
* **Last-Writer-Wins (LWW) with Conflict Resolution**: Figma avoids CRDT overhead for spatial positions by executing debounced server sync with monotonic sequence counters.
...
To ensure zero data loss during network disconnections, all client mutations are written to an **IndexedDB Local Mutation Queue** before or concurrently with network delivery.
...
Tradebook implements a centralized **Command Pattern Architecture** providing unified `Cmd+Z` (Undo) and `Cmd+Shift+Z` / `Cmd+Y` (Redo) capabilities across both tabular data grids and visual workflow canvases.
...
A rigorous evaluation of leading 2026 local-first sync engines against Tradebook's technical requirements:
...
To protect the browser main thread from locking up (0 FPS) during market-open WebSocket event storms (e.g. 5,000 incoming `LIVE SELECT` events/sec), incoming WebSocket push messages pass through an RxJS sliding-window buffer. 

Incoming events are collected into 50ms time windows (`bufferTime(50)`). Buffered events are reconciled into the TanStack Query / DB cache in a single microtask batch, bounding React re-renders to at most 20 FPS during peak message bursts while maintaining sub-100ms UI responsiveness:
...
Tradebook requires high-density tabular views capable of displaying 10,000 to 100,000+ data rows with inline editing, keyboard cell traversal, and column sorting without main-thread stutter.
...
When integrating `@dnd-kit` (for sorting and drag-and-drop elements) inside `@xyflow/react` (React Flow canvas viewports), a critical coordinate desynchronization occurs. 

React Flow applies viewport transformation via CSS scale and translate properties:
`transform: translate(tx, ty) scale(zoom);`

`dnd-kit`'s default `PointerSensor` relies on `window.getBoundingClientRect()`, which measures unscaled screen coordinates. Consequently, when the canvas zoom is not `1.0` (e.g., zoomed out at `0.5x` or in at `1.5x`), dragging a card or node causes the dragged element to detach from the cursor, traveling faster or slower than mouse pointer movement!
...
To prevent state fragmentation and race conditions, Tradebook establishes explicit boundaries for state ownership across three specialized state engines:
...
The following master matrix synthesizes both **Local-First Sync Engines** and **Table Rendering Engines** across 9 critical system engineering dimensions:
...
Following the strategic recommendations outlined in Tradebook’s architecture review (`alternatives/recommendation.md`), Tradebook adopts a **Two-Tiered Architecture Strategy**:
...
2. **Tabular Grid Virtualization**: **TanStack Table + TanStack Virtual** (unconstrained React styling, minimal 27KB bundle size, WAI-ARIA accessible).
...
5. **State Management Core**:
   - **Zustand**: Ephemeral UI focus and layout switches.
   - **XState**: Canvas workflow state machines.
   - **TanStack Query + TanStack DB**: Primary domain entity cache and optimistic live mutation queue.
...
- Wire Debezium CDC Outbox worker stream to SurrealDB read engine and WebSocket `LIVE SELECT` feeds.
- Implement client RxJS 50ms sliding-window WebSocket event throttling (`bufferTime(50)`) to protect React render performance during message storms.

#### Phase 2: Command Pattern Undo/Redo Engine & IndexedDB Queue (Weeks 4 - 6)
- Implement `LocalMutationQueueManager` using `idb` for offline action queueing, complete with `compactAndGetBatch()` mutation compaction.
...
- Construct the `UndoRedoStack` class and connect `Cmd+Z` / `Cmd+Shift+Z` keyboard shortcut listeners.
...
- Integrate `TanStack Table` and `@tanstack/react-virtual` for data table views (supporting 100,000 rows).
- Deploy `ZoomAwareDndContext` with `ZoomAwareDragOverlay` (`transform: scale(${zoom})`) and `createZoomModifier(zoom)` across React Flow workflow node editors to eliminate dnd-kit scale desync.
...
- Install `tanstack-db` and `tanstack-db-surrealdb` adapter.
- Pilot TanStack DB reactive collections on the Kanban board slice to manage card tags and column relations via `d2ts` differential dataflow.
- Conduct stress tests measuring memory consumption, frame rates, and latency under 5,000 concurrent live mutations.
- Validate Decision A performance against Decision B trigger criteria.

---

## 6. Document Verification & Compliance Checklist

- [x] **Sub-100ms CRUD & Perceptual UX**: Fully specified with benchmark analysis of Linear and Twenty CRM.
- [x] **PostgreSQL Primary Write Authority**: Eliminated direct SurrealQL writes from endpoints; routed writes through PostgreSQL atomic transactions with CDC outbox push to SurrealDB `LIVE SELECT`.
- [x] **WebSocket Throttling Engine**: Specified RxJS `bufferTime(50)` sliding-window batching for incoming WS events to preserve 60 FPS UI responsiveness.
- [x] **IndexedDB Queue Compaction & Batch Sync API**: Provided `LocalMutationQueueManager` `compactAndGetBatch()` and single bulk sync endpoint (`POST /api/v1/mutations/batch`).
- [x] **Recursive RFC 6902 3-Way Merge & Stable ULIDs**: Aligned merge conflict resolution with Pillar 1 (`mergeEngine.ts`), using stable ULID keys for collection items and conflict path isolation under `FAIL`.
- [x] **Command Pattern Undo/Redo Engine**: Detailed action interfaces, stack implementation, and state handlers provided.
- [x] **Local-First Sync Engine Matrix**: Evaluated TanStack DB, PowerSync, ElectricSQL, Replicache, and Zero across 12 core dimensions (including Memory footprint per 10k items, Reconnection bandwidth cost, and Multi-tab web lock protocols).
- [x] **Optimistic Write Sequence Diagrams**: Rendered in both Mermaid and ASCII formats showing PostgreSQL write authority, CDC outbox worker, and WS live push reconciliation.
- [x] **Virtualized Table Grid Comparison**: Compared AG Grid, TanStack Virtual, and Canvas (Glide Data Grid).
- [x] **React Flow + dnd-kit Zoom Translator**: Identified scale desync defect and provided full `ZoomAwareDragOverlay` production TypeScript solution with `transform: scale(${zoom})`.
- [x] **Unified State Strategy**: Unified Zustand, XState, and TanStack Query / DB with complete TypeScript bridge code.
