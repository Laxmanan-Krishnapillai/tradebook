...
Follow-up review with a **data-visualization lens** (dashboards, charts, KPIs, audit-history timelines, real-time tick updates). Both libraries were re-examined for shipped chart primitives, data binding, dashboard layout, realtime fit, and data-exposure risk.
...
- **openui** has real, working chart primitives bound to Recharts v2 (a D3 rewrite is in flight, pre-1.0, flagged "no unit tests"). Data flows through `Query` tool calls so raw ticket/trading data is **never sent to the model** — the LLM only emits query references. Runtime reactivity (`$variables` re-trigger queries) means updates after the initial render are LLM-free. Costs: ~4.9s initial full-render latency (60 tok/s benchmark), ~0.3s incremental patches, heavy bundle (Recharts + Radix + lucide + markdown), per-session non-deterministic layout with no dashboard-persistence primitive, pre-1.0 API churn.
- **json-render** is a spec/registry framework with **no charting out of the box** — its own demo dashboard hand-writes a `chart.tsx` wrapping Recharts and registers it. Its cleanest data path (`$state` → controlled store) keeps data out of the LLM but re-renders whole specs. `@json-render/codegen` is an immature scaffold exporter (full Next.js project), not a spec→committed-component pipeline. Verdict: keep it out of the viz path.

**Viz recommendation:** keep the hand-written 3-tier component approach (KPI cards / ECharts OLAP / Lightweight Charts ticks) as committed, agent-written React components fed by TanStack Query + SignalR. If an agent-authored dashboard surface is ever wanted, openui is the candidate to pilot for **ad-hoc/exploratory dashboards and audit-history timelines only** — one LLM render, then client-side updates — never the production realtime dashboard.

---

## 1c. Recommended Custom Visualization Stack (2026)

Four parallel research agents evaluated the current (mid-2026) landscape. Result: a **three-tier committed-component stack**, one maintained OSS dependency per tier, all MIT/Apache-2.0, data stays client-side.
...
4. **Dropped from the original plan:** Tremor (sunset), custom WebGL context-pool engine and the 8-context/512MB claims (canvas-only is simpler and honest), `echarts-for-react`, DuckDB-WASM edge (out of bootstrap scope).
...
**What it is**: A generative UI framework. At runtime an LLM composes the interface in *OpenUI Lang* (compact line-oriented DSL); the client-side `<Renderer>` progressively parses the stream and maps it onto a **Zod-typed component library you register**. npm packages `@openuidev/react-lang`, `@openui/react-ui`, `@openui/lang-core`, `@openui/cli`; ~1M downloads; MIT; best features (validation, observability, rollback) gated behind commercial "OpenUI Cloud".

**Applicability findings:**
- **React 19 / Vite / TS**: Compatible (peer `react ^18.3 || ^19`, TS 5.9, Zod 3/4, dual ESM/CJS).
- **TanStack Query CRUD**: Conceptual conflict — OpenUI's data model is "model chooses what to fetch" (MCP/tools), a second non-deterministic server-state path that bypasses query-key discipline, invalidation, and TypeGen types.
- **AI-agent codegen workflow**: **Biggest mismatch.** UI is generated per-session by the LLM in the browser → runtime dependency, not reviewable in git, not reproducible, per-render token/latency cost. Inverts the committed-code + human-review loop.
...
- **Security**: Reasonably safe by default (only registered, Zod-validated components compose; React escapes text). Residual risk via `MarkDownRenderer` (markdown→HTML XSS classic), URL/image props, and any self-registered component accepting `dangerouslySetInnerHTML`.

**Verdict: Avoid for bootstrap.** Narrow optional later use: an "AI insight" chat surface fed TanStack data via props, streaming OpenUI Lang over SignalR, with the markdown/URL components excluded.

---

## 3. json-render (vercel-labs/json-render)

**What it is**: A generative UI framework. You define a **catalog** of allowed components + actions (Zod-validated props); an LLM generates a **JSON spec** (element tree) constrained to the catalog; `<Renderer>` renders it. Packages: `@json-render/core` (schemas/catalogs/prompts/streaming), `@json-render/react` (`StateStore`, `$state`/`$bindState` expressions), `@json-render/shadcn` (36 prebuilt shadcn/Radix components), `@json-render/codegen` (exports specs as standalone React code), `@json-render/mcp` + bundled `skills/*.md`. Apache-2.0, ~586k weekly downloads, pre-1.0 with fast API churn.

**Applicability findings:**
- **React 19 / Vite / TS**: Compatible (`react ^19.2.3`, `zod ^4`, ESM+CJS+d.ts, plain npm lib, no build changes).
- **DTO→CRUD**: **No automatic generation.** It renders *specs* (catalog component trees), not raw DTO data. Forms/tables are authorable as specs (Input/Select/Textarea, `$bindState`, `validateForm`, `repeat`), but there is no RJSF-style JSON-Schema→form/table generator, and TypeGen types will not map to specs by themselves — you'd build a catalog + spec-mapping layer.
- **AI-agent codegen**: **Excellent fit — the project's core thesis.** The catalog is an agent guardrail; `catalog.prompt()` builds the system prompt; shipped `SKILL.md` files + MCP target AI clients; `@json-render/codegen` emits standalone React components with **zero runtime dependency** — ideal for commit + human review.
...
- **Security**: Strong — allowlist catalog means only your components render, props Zod-validated, no raw-HTML by default. Only risks if you author a component that inlines untrusted HTML or render unvalidated specs.
- **Gotchas**: 0.x breaking churn (spec format/props already rewritten once); `@json-render/shadcn` drags in Radix + Tailwind (a bootstrap styling decision); requires an LLM endpoint for any generative flow (the bootstrap currently has none); releases can lag across packages (0.19.0 shipped core-only).

**Verdict: Evaluate-more — do not adopt as the CRUD runtime.** Guardrail- and codegen-first design is a superb fit for agent-built UI, but it's a ~6-month-old 0.x framework and it is not a schema-driven CRUD/forms library.

---

## 4. Recommendation for the Tradebook Bootstrap

1. **Build biotickets CRUD deterministically** — React 19 + TanStack Query + TypeGen types + shadcn (or equivalent), with SignalR-driven invalidation. This is the resilient path and matches the "single source of truth" contract discipline from the adversarial review (`review/adversarial-tasklist-review.md`).
2. **Build data visualization deterministically** — committed React chart components fed by TanStack Query + SignalR, using the three-tier stack in §1c: Recharts v3 (via shadcn `chart`) for KPI/simple charts, Apache ECharts 6 (tree-shaken, direct-init wrapper) for OLAP/large series, TradingView Lightweight Charts v5 (or uPlot) for realtime ticks. No generative layer on the production/realtime dashboard path. Start Tier 1 only, add tiers when actually needed.
3. **Pilot json-render later, in codegen mode only**, for a scoped AI-authored surface (e.g., reporting/dashboard widgets or ticket-summary cards):
   ...
   - Emit specs → export via `@json-render/codegen` → **commit the generated React code** (reproducible, reviewable, zero runtime dependency).
   - Treat every spec as data (validate before render), pin versions, keep an escape hatch to plain React.
   - Note: for visualization specifically, json-render ships **no chart components** — you must register your own, and inline `data` props leak raw data to the model, so bind charts via `$state`/controlled stores only. It is **Avoid** for the viz path.
4. **Skip openui for CRUD**, but keep it as the one generative candidate worth a pilot for **ad-hoc/exploratory dashboards and audit-history timelines** — it ships real chart primitives and keeps raw data out of the LLM via `Query` tool calls. Scope it to one LLM render + client-side updates; never the production realtime surface. Reassess its pre-1.0 chart-engine churn before committing.
