...
> **DESCOPE NOTICE (2026-08-06)** — per [`architecture/decision-log.md`](../architecture/decision-log.md): remove Merkle/WORM verification (**D6** — the C# and SQL roots were mathematically incompatible anyway), all NATS checks (**D2**), `/api/v1/mutations/batch` verification (**D5**), and the 35k req/sec gate (**D10**). `AllowAnonymous()` in this spec's examples must become JWT policies (**D11**). Verify scripts must never wrap checks in `|| true` — a check that cannot fail is not a check. All cross-links must resolve against the post-descope file set.
...
### 1.1 Objectives
1. **End-to-End System Integration**: Seamlessly connect all Tradebook layers—PostgreSQL 17 System of Record, .NET 9 Native AOT FastEndpoints REPR API, NATS JetStream Outbox Processor, SignalR Core MessagePack Binary Streaming Hub, React 19 Local-First Snappy SPA UI, and the 3-Tier Custom Visualization Engine.
2. **Bi-Temporal Cryptographic Audit Verification**: Implement an RFC 6962 compliant Merkle Tree Verification Engine that validates point-in-time state records stored in PostgreSQL `TSTZRANGE` audit logs against S3 Object Lock COMPLIANCE WORM Parquet archives.
3. **Deterministic 3-Way Merge Resolution**: Author `perform3WayMerge` in TypeScript for client-side local-first state reconciliation, handling concurrent offline edits, ULID entity tracking, and command stack undo/redo operations with non-destructive conflict isolation (`FAIL` on overlapping scalar mutations).
4. **Full Platform Production Launch Runbook & Environment Verification**: Provide automated pre-flight environment checks, zero-downtime rolling update deployment workflows, Caddy reverse proxy TLS orchestration, and container health probes (`/health/live`, `/health/ready`, `/health/detail`).
5. **Continuous Agent Verification Protocol & Sentinel Master Acceptance Criteria**: Establish automated CI/CD guardrails (TypeGen DTO zero-drift, ArchUnitNET vertical slice boundaries, Stryker mutation testing ≥80%, Playwright E2E suites, NBomber 35,000 req/sec benchmark) and a 10-domain Sentinel Master Acceptance Criteria Matrix.

### 1.2 Scope
- **In-Scope**: End-to-end topology wiring, payload contracts, Merkle audit engine implementation, client 3-way merge engine, environment boot scripts, health probe endpoints, production deployment runbooks, agent verification protocols, sentinel acceptance matrix, and anti-cheating verification mandates.
...
- **Task 02**: .NET 9 Native AOT Web API with FastEndpoints REPR slices, EF Core 9, Dapper, and `HybridCache`.
...
- **Mechanism**: `NpgsqlDataSource` connection pooling combined with Dapper (high-speed reads) and EF Core 9 (command tracking).
...
- **Mechanism**: Polling Outbox Processor using `.NET 9 System.Threading.Channels` bound to a NATS JetStream stream named `TRADEBOOK_EVENTS`.
- **Stream Config**: Max age 7 days, storage File, replicas 3 (in growth/scale tiers).
...
- **Error Fallback**: Exponential backoff retry with dead-letter queue table `outbox_dead_letter` after 5 failed attempts.

#### 3. Boundary C: NATS JetStream -> SignalR Core Hub (MessagePack Push)
- **Mechanism**: SignalR Core Hub subscribes to NATS JetStream events and broadcasts binary MessagePack frames to connected client websocket rooms.
...
- **Throttling Pipeline**: Incoming updates pass through an RxJS sliding window `bufferTime(50)` operator. Updates coalesce over 50ms intervals before dispatching to TanStack Query / Zustand stores, preventing main-thread re-render churn during high-frequency market bursts (capping UI updates at 20 FPS).
- **Offline Mutation Queue**: If offline, mutations are staged in IndexedDB (`Dexie.js`) with `status: 'PENDING'`. Upon network reconnection, `syncMutationQueue()` drains pending actions in batch (`POST /api/v1/mutations/batch`).
...
- **Worker & GPU Governance**:
  - Large datasets (>100,000 data points) are downsampled off-main-thread in a Web Worker using the **Largest-Triangle-Three-Buckets (LTTB)** algorithm.
  - Render commands offload to background threads using `OffscreenCanvas`.
  - `WebGLContextPoolManager` enforces a hard cap of **maximum 8 active canvas contexts per tab**.
...
Cold audit logs exported to AWS S3 Parquet buckets use **S3 Object Lock in COMPLIANCE mode (7-year mandatory retention)**. To prevent leaf-duplication attacks (CVE-2012-2459), the Merkle Tree Verification Engine strictly adheres to **RFC 6962 (Certificate Transparency)**:
...
3. **Odd Node Carry-Up**: If a level has an odd number of nodes, the last node is carried up directly to the next tree level without duplication.
...
When multiple users or local offline mutation queues modify entities concurrently, `perform3WayMerge` reconciles state between the Base Version ($O$), Local Branch Head ($A$), and Remote Branch Head ($B$).
...
3. **Conflict Resolution Policy**:
   ...
   - Overlapping scalar mutations to the exact same property key trigger conflict isolation: operation status set to `FAIL`, generating a formal merge conflict object for user intervention.
4. **Command Stack Integration**: Merged patches push onto `UndoRedoStack` to support `Cmd+Z` (Undo) and `Cmd+Shift+Z` (Redo).
...
The matrix below maps every domain of the Tradebook platform to its functional requirement, target SLA, automated verification command, pass criteria, and audit verification step.
...
1. **Step 1: Database & Audit Verification Setup**:
   - Apply `001_initial_schema.sql` and `002_timescaledb_hypertables.sql`.
...
2. **Step 2: Backend API & Health Probes**:
   - Implement `LiveHealthEndpoint.cs` and `ReadyHealthEndpoint.cs` in `Tradebook.Api.Endpoints`.
...
3. **Step 3: Client 3-Way Merge Engine**:
   ...
   - Write unit tests in `src/Frontend/src/lib/merge/__tests__/perform3WayMerge.test.ts` covering non-overlapping merges, scalar conflicts, and patch generation.

4. **Step 4: Verification Scripts & Execution Tooling**:
   - Author `scripts/platform-verify.sh` and `scripts/platform-verify.ps1`.
...
5. **Step 5: Master Verification Run**:
   ...
   - Execute `./scripts/platform-verify.sh` and record results in handoff report.
