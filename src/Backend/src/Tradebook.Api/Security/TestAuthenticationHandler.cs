    ...
    : AuthenticationHandler<AuthenticationSchemeOptions>(options, logger, encoder)
{
    ...
    protected override Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        raw ??= Request.Query["access_token"].ToString() is { Length: > 0 } queryToken ? queryToken : null;
        if (raw is null) return Task.FromResult(AuthenticateResult.NoResult());
        try
        {
            ...
            if (subject is null || !Guid.TryParse(subject, out _)) return Task.FromResult(AuthenticateResult.Fail("Test token subject must be a UUID."));
            claims.RemoveAll(c => c.Type is "oid" or "tid" or "tradebook_tenant" or "scp");
            claims.AddRange([new("oid", subject), new("tid", entra.Value.TenantId), new("tradebook_tenant", entra.Value.TenantId), new("scp", "access_as_user")]);
