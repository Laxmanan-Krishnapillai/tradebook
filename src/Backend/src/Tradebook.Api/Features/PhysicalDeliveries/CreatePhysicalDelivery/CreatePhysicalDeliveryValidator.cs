...
public sealed class CreatePhysicalDeliveryValidator : Validator<CreatePhysicalDeliveryRequest>
{
    public CreatePhysicalDeliveryValidator()
    {
        ...
        RuleFor(request => request.BookType)
            .Must(value => value is "Sourcing" or "Sales" or "Intercompany");
        ...
        RuleFor(request => request.VolumeNominatedMwh)
            .GreaterThanOrEqualTo(0)
            .When(request => request.VolumeNominatedMwh.HasValue);
