...
public sealed class UpdateTaxTariffEndpoint(ITaxTariffRepository repository)
    : Endpoint<UpdateTaxTariffRequest, TaxTariffDetailsDto>
{
    public override void Configure()
    {
        Put("/api/v1/tax-tariffs/{taxTariffId}");
    ...
    public override async Task HandleAsync(UpdateTaxTariffRequest request, CancellationToken ct)
    {
        ...
        var current = await (repository.GetByIdAsync(request.TaxTariffId, ct)).ConfigureAwait(
            false
        );
        if (current is null)
        {
