...
/** coss UI's Base UI tabs, normalized to Tradebook's token-locked CSS surface. */
...
export function TabsList({ className, variant = 'default', children, ...props }: ListProps) {
  return <TabsPrimitive.List className={classes('ui-tabs-list', className)} data-slot="tabs-list" data-variant={variant} {...props}>{children}<TabsPrimitive.Indicator className="ui-tabs-indicator" data-slot="tab-indicator" /></TabsPrimitive.List>;
}
