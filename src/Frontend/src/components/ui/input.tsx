...
export function Input({ appearance = 'default', className, size = 'default', type, ...props }: InputProps) {
  const [focused, setFocused] = useState(false);
  const reduceMotion = useReducedMotion();

  return (
    <m.span
      animate={appearance === 'default' && focused && !reduceMotion ? { scale: 1.003, y: -1 } : { scale: 1, y: 0 }}
