...
function EntityEditor<TChanges extends object>({ initialChanges, fields, onSave, onDelete, deleteLabel, extraAction }: {
  ...
  return <details><summary>Actions</summary><div className="grid grid-cols-4 items-center gap-2 max-[800px]:grid-cols-2">{fields.map((field) => <FieldInput key={field.key} field={field} value={changes} onChange={setChanges} />)}<button type="button" onClick={() => onSave(changes)}>Save</button>{extraAction && <button type="button" disabled={extraAction.disabled} onClick={extraAction.run}>{extraAction.label}</button>}<button type="button" className="bg-red-700" onClick={onDelete}>{deleteLabel}</button></div></details>;
}
...
function DomainCrudPage<T extends Versioned, TCreate extends object, TChanges extends object>(props: CrudPageProps<T, TCreate, TChanges>) {
  ...
  return <section><header className="mb-6 flex items-start justify-between gap-4 max-[800px]:flex-col max-[800px]:items-stretch"><div><p className="mb-1 text-xs font-extrabold uppercase tracking-widest text-gray-600">{props.eyebrow}</p><h2>{props.title}</h2><p>{history.data ? `${history.data.totalCount} records` : `Loading ${props.title.toLowerCase()}…`}</p></div><button type="button" onClick={() => setShowCreate(true)}>Create</button></header>{props.feedback.error && <p role="alert" className="rounded-lg bg-red-100 p-3 text-red-900">{props.feedback.error}</p>}{history.isError && <p role="alert">Unable to load {props.title.toLowerCase()}.</p>}{!history.isError && <VirtualizedDataTable data={history.data?.items ?? []} columns={columns} getRowId={props.idOf} />}{history.data && <nav className="flex flex-wrap items-center gap-2" aria-label={`${props.title} pages`}><button type="button" disabled={page === 1 || history.isFetching} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</button><span>Page {page}</span><button type="button" disabled={!history.data.hasNextPage || history.isFetching} onClick={() => setPage((value) => value + 1)}>Next</button></nav>}{showCreate && <section className="fixed inset-0 z-20 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true" aria-label={`Create ${props.title}`}><form onSubmit={(event) => void submitCreate(event)}><h3>Create {props.title}</h3>{props.createFields.map((field) => <FieldInput key={field.key} field={field} value={createRequest} onChange={setCreateRequest} />)}<div className="flex flex-wrap items-center gap-2"><button type="button" className="bg-gray-200 text-gray-800" onClick={() => setShowCreate(false)}>Close</button><button type="submit" disabled={props.createMutation.isPending}>Create</button></div></form></section>}{props.feedback.conflict && <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/50 p-4"><ConflictDialog entityId={props.feedback.conflict.id} serverState={props.feedback.conflict.serverState} attemptedChanges={props.feedback.conflict.attempted} onClose={() => props.feedback.setConflict(undefined)} /></div>}</section>;
}
