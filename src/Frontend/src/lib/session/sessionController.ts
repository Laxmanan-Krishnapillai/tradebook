...
import { clearMutationVersions } from '../mutations/mutationCoordinator';
...
function invalidateSession(): void { useAuthStore.getState().clearSession(); clearMutationVersions(); }
export function beginSession(session: AuthSession, returnPath?: unknown): Promise<void> { return serialize(async () => { invalidateSession(); await reset(); useAuthStore.getState().setSession(session); await navigation?.invalidate(); await navigation?.navigateTo(validateInternalReturnPath(returnPath) ?? '/deliveries'); }); }
export function endSession(reason: SessionEndReason, options: { navigate?: boolean } = {}): Promise<void> { return serialize(async () => { invalidateSession(); await reset(); msalInstance.setActiveAccount(null); await navigation?.invalidate(); if (reason === 'logout' && options.navigate !== false) { await msalInstance.logoutRedirect(); return; } if (options.navigate !== false) await navigation?.navigateToLogin(); }); }
