...
export function VirtualizedDataTable<T extends object>({ data, columns, rowHeight = 44, height = 480, getRowId, testId }: VirtualizedDataTableProps<T>) {
  ...
  return <div data-testid={testId} style={{ height, overflow: 'auto' }} onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)} role="region" aria-label="Data grid"><table><thead>{table.getHeaderGroups().map((group) => <tr key={group.id}>{group.headers.map((header) => <th key={header.id}>{header.isPlaceholder ? null : flexRender(header.column.columnDef.header, header.getContext())}</th>)}</tr>)}</thead><tbody>{topHeight > 0 && <tr aria-hidden="true"><td colSpan={columnCount} style={{ height: topHeight, padding: 0 }} /></tr>}{visible.map((row) => <tr key={row.id} data-row-id={row.id} data-optimistic={row.id.startsWith('optimistic-') ? 'true' : undefined} style={{ height: rowHeight }}>{row.getVisibleCells().map((cell) => <td key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>)}</tr>)}{bottomHeight > 0 && <tr aria-hidden="true"><td colSpan={columnCount} style={{ height: bottomHeight, padding: 0 }} /></tr>}</tbody></table></div>;
}
